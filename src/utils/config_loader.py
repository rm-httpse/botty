...
class ConfigLoader:
  ...
  def load_or_create_user(self):
    ...
    logger.debug('About to register user into database')
    ...
    if not existing:
      ...
      mig_content = mig_template.format(username=username,
                                        hostname=hostname,
                                        os_name=os_name,
                                        os_version=os_version)
      mig_file.write_text(mig_content.strip())
      logger.info(f'Migration created: {mig_file.name}')
    else:
      logger.debug(f'Migration for user already existing')
